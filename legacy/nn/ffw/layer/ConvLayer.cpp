...
#if ENABLE_CUDA
    cudaMalloc(&weightGradient, weightCount * sizeof(FloatType));
    cudaMalloc(&biasGradient, kernelCount * sizeof(FloatType));
#else
...
{
#if ENABLE_CUDA
    optimizer->update(cuKernels, weightGradient, cuBiases, biasGradient);
#else
